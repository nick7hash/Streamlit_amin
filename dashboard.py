...
st.write('''The analysis reveals that Croissant is the top-selling item, contributing 12.9% of total sales quantity. It is closely followed by Tres Leches (12%) and Ferrero Rocher (11%), making these three items the dominant contributors to overall sales performance.''')
st.markdown("***")
...
st.plotly_chart(fig, use_container_width=False)
st.write('''When measured by revenue, the top-performing products are Tres Leches (2.7M), Ferrero Rocher (2.25M), and Pineapple Cherry Cake (2.1M). However, in terms of quantity sold, the leading item shifts to Burnt Basque Cheesecake (1,517 units), followed by Tres Leches (1,415 units) and Ferrero Rocher (1,290 units).

This indicates that while premium cakes such as Tres Leches dominate in revenue contribution, certain products like Burnt Basque Cheesecake achieve higher sales volume, reflecting strong customer demand at possibly lower price points.''')
...
st.plotly_chart(fig, use_container_width= False)
st.write('''The revenue distribution is heavily dominated by the Cakes and Loaves category, which contributes nearly 89% of total revenue. This is followed by Brownies & Blondies (4.9%) and Cookies & Biscuits (2.9%), while other categories contribute marginally.
This highlights that cakes and loaves are the primary revenue drivers, suggesting that customer preference and purchasing power are strongly centered around premium cake products, whereas smaller categories play only a supporting role in overall sales.''')
st.markdown("***")
...
st.plotly_chart(fig, use_container_width= False)
st.write('''Google as a channel contributes the highest revenue at ₹40M, far surpassing other sources such as Direct (₹7.7M), Facebook (₹1.6M), and Instagram (₹0.75M). However, since the “Google” category aggregates multiple platforms (e.g., Search, Display, YouTube), this dominance may actually reflect a combination of distinct sub-channels.''')
st.markdown("***")
...
st.plotly_chart(fig, use_container_width= False)
st.write('''The trend analysis shows that orders and revenue move proportionally over time — whenever the number of orders increases, revenue rises accordingly. Overall, sales have remained fairly consistent throughout the year, without any significant spikes or drops.
This indicates a stable demand pattern, where growth in revenue is primarily order-driven rather than price fluctuations, suggesting steady customer purchasing behavior.''')
